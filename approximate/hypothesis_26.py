...
class SpectrumGenerator:
    ...
    def __next__(self):
        with self.lock:
            data = [one_random_mixed_spectrum() for i in range(self.batch_size)]
            Y, Z = map(numpy.array, zip(*data))

            return Y, Z


class NoisedSpectrumGenerator:
    def __init__(self, batch_size=32, standard_deviation=1):
        self.batch_size = batch_size
    ...
    def __next__(self):
        with self.lock:
            data = [one_random_mixed_spectrum() for i in range(self.batch_size)]
            Y, Z = map(numpy.array, zip(*data))
            Y = random.normal(Y, self.standard_deviation)
...
def compile_model(model):
    ...
    model.compile(
        loss=loss,
        optimizer=optimizer,
        metrics=[true_accuracy, area_accuracy, signal2noise_accuracy])
...
def main():
    ...
    #generator = SpectrumGenerator(batch_size)
    generator = NoisedSpectrumGenerator(batch_size, 0.01)
